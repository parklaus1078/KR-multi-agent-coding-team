...
- `planning-materials/fe-test-cases/` — 컴포넌트/훅별 테스트 케이스 목록
- `planning-materials/fe-ui-requirements/` — 각 화면의 인터랙션 및 상태 정의
- `planning-materials/be-api-requirements/` — 연동 API의 Response 스키마 → **MSW mock 데이터 생성에 활용**
- `applications/fe-project/src/` — 실제 테스트 대상이 되는 컴포넌트/훅의 시그니처 및 구조
...
```text
0-a. applications/fe-project/tests/setup.ts         — Testing Library + MSW 초기화
0-b. applications/fe-project/tests/mocks/server.ts  — MSW 서버 설정
```
...
1. applications/fe-project/tests/mocks/api/{domain}.ts
   — planning-materials/be-api-requirements/에 정의된 ApiResponse<T> 스키마를 그대로 반영한 mock 응답 데이터
